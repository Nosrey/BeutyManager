...
function CrearProducto({ setForm, visible, setProductos, productos, filtrarProductos, categorias, setCategorias }) {
    ...
    const imagenNotFound = 'https://www.freeiconspng.com/thumbs/no-image-icon/no-image-icon-6.png'
    ...
    return (
        <div className={visible ? "fixed bg-white w-96 top-0 border-2 rounded-md border-l-0 hover:border-sky-200 overflow-auto max-h-screen p-4 pt-1" : 'hidden'}>



            <button className='bg-red-600 text-white absolute top-2 right-2 px-1.5 font-black hover:bg-red-300 text-xl' onClick={cerrar}>X</button>
            <div className="">
                <form onSubmit={handleSubmit} autoComplete="off" className='flex flex-col items-center'>
                    <div className="input-container">
                        <label className="text-xl font-semibold text-center">Nombre del producto</label>
                    ...
                    <div className="input-container">
                        <label className="text-xl font-semibold text-center">Cantidad Disponible</label>
                        <input type="text" name="pstock" placeholder='Cantidad...' className=" mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-lg shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500" required />

                    </div>
                    <div className="input-container">
                        <label className="text-xl font-semibold text-center">Precio del producto</label>
                    ...
                    <div className="input-container flex flex-col items-center">
                        <label className="text-xl font-semibold text-center">Categorias</label>
                        <input value={pcategory} onChange={handlePcategory} type="text" name="pcategory" placeholder="Categorias..." className=" mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-lg shadow-sm placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500" />

                        <p>{(finalElement(pcategory).toLowerCase() === filterCategories.toLowerCase() && finalElement(pcategory).length) ? <p className='my-1'></p> : (filterCategories === notFound) ? <p className='my-1 text-gray-500 italic'>{filterCategories}</p> : <div className='my-1'><p className='inline text-gray-500 italic'>Sugerencia: </p><p className='inline'>{primeraMayuscula(filterCategories)}</p></div>}</p>
                    ...
                    <img src={image} alt='product' className='w-28 inline py-4' />
                    <button type='submit' className='bg-sky-500 hover:bg-sky-600 text-white font-bold py-2 px-4 rounded text-xl'>Crear producto</button>
                </form>
            </div>
        </div>
    )
